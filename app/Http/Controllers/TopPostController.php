...
class TopPostController extends Controller
{
    public function main (Request $request) {
      ...
      switch ($mode) {
        case 'new': $items = $posts->select(DB::raw("*, topposts.id AS id, likes/views AS popular"))->where('likes', '>', 100)->where('action_time', '>', date('U')-60*60*13)->orderBy('popular', 'desc')->paginate(100)->withQueryString(); break;
