...
@endisset
@if (!empty($info['found']) && @isset ($info['demo']))
  <div class="w-100 alert alert-warning">
    Демо-поиск: максимум <b>10</b> групп; для просмотра 1000 групп, <a href="{{ route('tarifs') }}">оплатите полный доступ</a>. В разделе <a href="{{ route('groupsearch') }}">"Группы с открытой стеной"</a> можно скачивать до 100 000 групп.
  </div>
@endif
...
@include('inc.obsolete-token')
  ...
  @else
    @if(Session::has('vkid'))
      @isset ($info['demo'])
        <div class="w-100 m-0 alert alert-info"><strong><a class="alert-link" href="{{ route('download', 'temp\top1000_') }}">Скачать ТОП-1000 групп (демо-версия). Для скачивания полной версии файла выберите подходящий тариф</a></strong></div>
      @else
